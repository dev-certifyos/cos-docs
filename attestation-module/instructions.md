...
| ~~3~~ | ~~SFTP Exchange (inbound + outbound)~~ | ~~`sftp-exchange`~~ | — | **Retired 2026-09-08 (D2-38)** — folded into the single standalone directory accuracy doc below. Draft moved out 2026-09-09 → `platform/directory-accuracy/source-material/sftp-exchange.md`. |
| ~~4~~ | ~~External Source Ingestion Layer~~ | ~~`ingestion`~~ | — | **Retired 2026-09-08 (D2-38)** — folded into the single standalone directory accuracy doc below. Draft moved out 2026-09-09 → `platform/directory-accuracy/source-material/ingestion.md`. |
...
- Docs already ticked in §9 (cycle, portal-lane; sftp-exchange was ticked then, and left the series 2026-09-08) are re-checked against this decision; needed changes flow through the §8.1 amendment process (explicit flags, amend after review) — never silent edits.
...
- [x] ~~3. `modules/03-sftp-exchange/sftp-exchange.md`~~ — **retired 2026-09-08 (D2-38)**; was approved 2026-08-27 and rewritten 2026-09-05 (pending review). **Moved out of the series 2026-09-09** → `cos-docs/platform/directory-accuracy/source-material/sftp-exchange.md` (+ its companion), source material for the directory accuracy doc.
- [ ] ~~4. `modules/04-ingestion/ingestion.md`~~ — **retired 2026-09-08 (D2-38)**, never ticked. **Moved out of the series 2026-09-09** → `cos-docs/platform/directory-accuracy/source-material/ingestion.md` (+ its companion), source material for the directory accuracy doc.
- [ ] 5. `modules/05-backend/backend.md`
- [ ] 6. `modules/06-database-data-layer/database-data-layer.md`
- [ ] 7. `modules/07-ui/ui.md`
- [ ] 8. `modules/08-client-export/client-export.md` *(own doc — the earlier "fold into the ingestion/outbound doc" option died when that doc left the series, D2-38)*

**Related standalone design docs (not part of the attestation series; same template and house style):**

- [ ] `cos-docs/platform/directory-accuracy/directory-accuracy.md` — the **directory accuracy service (external-source ingestion, Candor program)**, decoupled from the attestation workflow 2026-09-08 (D2-38). One doc covering the whole vendor workflow: tenant-configurable selection query, scheduled export, SFTP transport, ingestion + dispositions, rejected-recommendations lookup, staging, review (approve/reject/skip), Sync Latest release under `candor:{tenantId}`. **To write — its companion `directory-accuracy-concepts.md` exists first (2026-09-09): concepts + pre-design analysis (service boundary, two deployables, language, database, scale stress test, industry patterns, open questions).** Source material = the retired SFTP-exchange and ingestion drafts, now at `cos-docs/platform/directory-accuracy/source-material/` (moved 2026-09-09), plus the `attestation-module/reference/candor-*` files. Open inputs: v2 O-1, O-17, O-18, O-19.

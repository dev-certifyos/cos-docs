...
**Service boundary:** the program is designed as its own external-source-ingestion service with a single standalone design document (replacing the former SFTP-exchange and ingestion module docs of the attestation series). Where its review surface lives (inside the Attestation Module UI application or its own) and whether it reuses the attestation module's Sync Latest machinery are open decisions (O-19).

### 6.4 Inbound delivery: file + manifest (manifest last)

> *Amended 2026-09-08 (D2-38): §6.4–§6.6 describe the vendor program's ingestion — they belong to the standalone external-source-ingestion service, not to the attestation module. The mechanics below still hold for that service.*
...
| O-9 | Client export contract | Covered in the **outbound/inbound ingestion module design doc**; cadence: **every 30 days**. → D2-13 |
...
4. Start the per-module design documents, in dependency order: Attestation Module Backend + data layer + database → Attestation Module UI → Scheduler/Outreach extensions → Client Export. The external-source-ingestion service (Candor program) gets **one standalone design document** outside this series (D2-38 — replaces the former SFTP Exchange and External Source Ingestion Layer module docs).
